...
export function getSafeDatabaseMetadata(): { host: string; database: string; configured: boolean } {
  if (!config.databaseUrl) {
    return { host: 'none', database: 'none', configured: false };
  }
  try {
    ...
    return { host: 'unparseable', database: 'unparseable', configured: true };
  }
}
...
export function loadSchemaSql(): { sql: string; source: string } {
  // Resolve ESM / CJS directory safely without throwing ReferenceError
  let currentDir = '';
  try {
  ...
  const candidatePaths = [
    currentDir ? path.join(currentDir, 'schema.sql') : '',
    currentDir ? path.join(currentDir, '..', 'src', 'server', 'db', 'schema.sql') : '',
    cjsDir ? path.join(cjsDir, 'schema.sql') : '',
    cjsDir ? path.join(cjsDir, '..', 'src', 'server', 'db', 'schema.sql') : '',
    ...
    try {
      if (fs.existsSync(candidate)) {
        ...
        if (sql && sql.trim().length > 0) {
          return { sql, source: candidate };
        }
      }
    } catch {}
  }

  // Fallback to embedded canonical schema to guarantee zero runtime failures
  return { sql: CANONICAL_SCHEMA_SQL, source: 'embedded_canonical_schema' };
}

export async function initDatabase(): Promise<boolean> {
  const activePool = getPool();
  if (!activePool) {
    console.warn('[Database] DATABASE_URL not provided. Running in memory / mock storage mode.');
    return false;
  }
  ...
  console.log(`[Database] Connecting to PostgreSQL at ${meta.host} (database: ${meta.database})...`);

  const client = await activePool.connect();
  try {
    // 1. Load schema SQL safely
    const { sql, source } = loadSchemaSql();
    console.log(`[Database] Applying schema from source: ${source}...`);
    ...
    for (const table of REQUIRED_TABLES) {
      ...
      if (!checkRes.rows[0]?.regclass) {
        throw new Error(`Database verification failed: Required table "public.${table}" does not exist.`);
      }
    }

    // 4. Verify 2026 registration sequence exists
    await client.query(`
      INSERT INTO registration_sequence (year, last_number)
      VALUES (2026, 0)
      ON CONFLICT (year) DO NOTHING;
    ...
    const seqRes = await client.query(
      `SELECT year, last_number FROM registration_sequence WHERE year = 2026`
    );
    if (seqRes.rows.length === 0) {
      throw new Error('Database verification failed: 2026 registration sequence row is missing.');
    }

    const currentSeq = seqRes.rows[0].last_number;
    console.log(`[Database] All ${REQUIRED_TABLES.length} tables and 2026 sequence verified (last_number: ${currentSeq}).`);
    console.log(`[Database] Neon PostgreSQL database is healthy and ready.`);

    isDbInitialized = true;
    return true;
  } catch (err: any) {
    isDbInitialized = false;
    console.error(`[Database Init Error] Failed to initialize PostgreSQL (${meta.host}/${meta.database}):`, err.message);
    throw err;
  } finally {
    client.release();
  }
...
let queryOverride: ((text: string, params?: any[]) => Promise<any>) | null = null;
let transactionOverride: (<T>(callback: (client: any) => Promise<T>) => Promise<T>) | null = null;

export function setDatabaseOverrides(overrides: {
  query?: ((text: string, params?: any[]) => Promise<any>) | null;
  withTransaction?: (<T>(callback: (client: any) => Promise<T>) => Promise<T>) | null;
} | null) {
  queryOverride = overrides?.query || null;
  transactionOverride = overrides?.withTransaction || null;
}
...
export async function withTransaction<T>(
  ...
  if (transactionOverride) {
    return transactionOverride(callback);
  }
  const activePool = getPool();
  if (!activePool) {
    throw new Error('DATABASE_URL is not configured on this server.');
